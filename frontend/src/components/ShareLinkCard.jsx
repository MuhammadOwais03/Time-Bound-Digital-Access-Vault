...
//   return (
//     <div
//       className={`relative bg-white/5 rounded-lg p-5 border transition-all ${
...
//             {/* Dropdown Menu */}
//             {showDropdown && (
//               <div className="absolute right-0 mt-2 w-48 bg-gray-900/95 backdrop-blur-lg rounded-lg border border-white/20 shadow-xl z-[9999] overflow-hidden">
//                 {/* Lock / Unlock */}
//                 <button
...
const EditShareModal = ({ share, onClose, onUpdated, showNotification }) => {
  const [expiresInDays, setExpiresInDays] = useState(
    share.expiresAt
      ? Math.ceil((new Date(share.expiresAt) - new Date()) / (1000 * 60 * 60 * 24)) + "d"
      : "never"
  );
  const [maxViews, setMaxViews] = useState(share.maxViews || "");
  const [password, setPassword] = useState(share.passwordHash || "");
  ...
  const handleUpdate = async () => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        setError(data.message || "Failed to update share link");
      }
    } catch (err) {
  ...
  return (
    <div className="fixed inset-0 bg-black/50 backdrop-blur-sm flex items-center justify-center z-[9999]">
      <div className="bg-gray-900 rounded-2xl p-8 border border-white/20 max-w-md w-full mx-4 shadow-2xl">
        <h2 className="text-2xl font-bold text-white mb-6">Edit Share Link</h2>
            ...
            <div>
              ...
              <div className="relative">
                <input
                  ...
                  onChange={(e) => setPassword(e.target.value)}
                  placeholder="Leave empty for no password"
                  className="w-full px-4 py-3 pr-12 bg-black/50 border border-white/20 rounded-lg text-white placeholder-purple-300/50 focus:outline-none focus:border-purple-400 transition-colors"
                />
                {password && (
                  <button
                    type="button"
...
const ShareLinkCard = ({ share, showNotification, onDelete }) => {
  const [isEditing, setIsEditing] = useState(false);
  const [isLocked, setIsLocked] = useState(share.isLocked);
  const [loadingAction, setLoadingAction] = useState(null);
  const [currentToken, setCurrentToken] = useState(share.token);
  ...
  const fullUrl = `https://time-bound-digital-access-vault-nine.vercel.app/share/${currentToken}`;
  const isExpired = shareData.expiresAt && new Date(shareData.expiresAt) < new Date();
  const isExhausted = shareData.remainingViews === 0;
  ...
  const handleLockToggle = async () => {
    ...
    try {
      const endpoint = isLocked
        ? `/api/share-links/shares/${shareData._id}/unlock`
        : `/api/share-links/shares/${shareData._id}/lock`;
      ...
      if (res.ok) {
        setIsLocked(!isLocked);
        setShareData({ ...shareData, isLocked: !isLocked });
        showNotification(
          `Share link ${isLocked ? "unlocked" : "locked"} successfully`,
          "success"
        );
      } else {
        const data = await res.json();
  ...
  const handleRegenerate = async () => {
    ...
    try {
      const res = await fetch(
        `${API_BASE_URL}/api/share-links/shares/${shareData._id}/regenerate`,
        {
          method: "PATCH",
          credentials: "include",
        }
      ...
      if (res.ok) {
        setCurrentToken(data.data.newToken);
        showNotification("Token regenerated! Old link is now invalid.", "success");
      } else {
        throw new Error(data.message || "Failed to regenerate token");
      }
    } catch (err) {
  ...
  const handleDelete = async () => {
    ...
    try {
      const res = await fetch(`${API_BASE_URL}/api/share-links/${shareData._id}`, {
        method: "DELETE",
        credentials: "include",
      });
  ...
  const handleUpdated = (updatedData) => {
    setShareData({ ...shareData, ...updatedData });
  };

  return (
    <>
      <div className="bg-gradient-to-br from-white/5 to-white/10 rounded-xl p-6 border border-white/10 hover:border-white/20 transition-all duration-300 shadow-lg">
        <div className="flex items-start justify-between gap-4">
          <div className="flex-1 min-w-0">
            {/* Status Badge */}
            <div className="flex items-center gap-2 mb-4 flex-wrap">
              {shareData.passwordHash ? (
                <span className="inline-flex items-center gap-1.5 px-3 py-1 bg-yellow-500/20 text-yellow-300 rounded-full text-xs font-medium">
                  <Lock size={14} />
                  Password Protected
                </span>
              ) : (
                <span className="inline-flex items-center gap-1.5 px-3 py-1 bg-green-500/20 text-green-300 rounded-full text-xs font-medium">
                  <Shield size={14} />
                  Open Access
                </span>
              )}
              ...
              {isLocked && (
                <span className="inline-flex items-center gap-1.5 px-3 py-1 bg-red-500/20 text-red-300 rounded-full text-xs font-medium">
                  <Lock size={14} />
                  Locked
                </span>
              )}
            ...
            {/* Share URL */}
            <div className="bg-black/30 rounded-lg p-3 mb-4 break-all">
              <p className="text-white/90 font-mono text-sm">{fullUrl}</p>
            </div>

            {/* Info Grid */}
            <div className="grid grid-cols-2 gap-4 text-sm">
              <div className="space-y-1">
                <p className="text-purple-300 font-medium">Expires</p>
              ...
              <div className="space-y-1">
                <p className="text-purple-300 font-medium">Views</p>
                <p className={`${isExhausted ? "text-red-400" : "text-white/80"}`}>
                  {shareData.remainingViews ?? "∞"} / {shareData.maxViews || "∞"}
                </p>
              </div>
            </div>
          </div>
          ...
          <div className="flex flex-col gap-2">
            <button
              onClick={copyToClipboard}
              className="p-2.5 bg-purple-500/20 hover:bg-purple-500/30 rounded-lg transition-all duration-200 group"
              title="Copy link"
            >
              <Copy size={18} className="text-purple-300 group-hover:text-purple-200" />
            </button>

            <div className="relative" ref={dropdownRef}>
              <button
                onClick={() => setShowDropdown(!showDropdown)}
                className="p-2.5 bg-white/10 hover:bg-white/20 rounded-lg transition-all duration-200"
                title="More options"
              >
                <MoreVertical size={18} className="text-white" />
              </button>
              ...
              {showDropdown && (
                <div className="absolute right-0 top-full mt-2 w-56 bg-gray-900 rounded-lg border border-white/20 shadow-2xl overflow-hidden z-50">
                  <button
                    onClick={handleLockToggle}
                    disabled={loadingAction === "lock"}
                  ...
                  >
                    ...
                    <span className="font-medium">{isLocked ? "Unlock Link" : "Lock Link"}</span>
                  </button>
                  ...
                  >
                    ...
                    <span className="font-medium">Regenerate Token</span>
                  </button>
                  ...
                  <button
                    onClick={handleDelete}
                    disabled={loadingAction === "delete"}
                    className="w-full px-4 py-3 text-left text-red-400 hover:bg-red-500/20 transition-colors flex items-center gap-3 disabled:opacity-50 disabled:cursor-not-allowed"
                  >
                    {loadingAction === "delete" ? (
